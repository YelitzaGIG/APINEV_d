...
(function () {
    ...
    // ── BASE URL ─────────────────────────────────────────────
    // Busca la meta en todo el documento (puede estar en <head> o en <body>)
    const metaTags = document.querySelectorAll('meta[name="base-url"]');
    const BASE = metaTags.length > 0 ? metaTags[0].content : '';
    ...
    if (btnPdf) {
        btnPdf.addEventListener('click', function () {
            if (!idAlumnoActivo || !idGrupoActivo || !idInstrumentoActivo) {
                toast('Selecciona grupo, instrumento y alumno antes de generar el PDF.', 'error');
                return;
            }
            const url = BASE
    ...
    function onSelectChange() {
        ...
        // Al cambiar selects se pierde la selección de alumno y reactivo
        idAlumnoActivo   = null;
        idReactivoActivo = null;

        // Limpiar selección visual de alumnos
        document.querySelectorAll('#tbody-alumnos tr').forEach(tr => tr.classList.remove('ev-fila-sel'));

        // Deshabilitar PDF porque ya no hay alumno seleccionado
        actualizarBotonPdf();
    ...
    document.addEventListener('click', function (e) {
        ...
        if (filaAlumno && !e.target.closest('.ev-btn-op')) {
            ...
            actualizarBotonPdf();   // ← se llama después de asignar el id
            return;
        }
    ...
    function configurarNavegacion(tbodyId, btnPrevId, btnNextId) {
        ...
        function seleccionar(tr) {
            ...
            if (tbodyId === 'tbody-alumnos') {
                ...
                actualizarBotonPdf();   // ← también se actualiza al navegar con Anterior/Siguiente
            } else {
